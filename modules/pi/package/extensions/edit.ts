...
async function generateDiffString(oldContent: string, newContent: string): Promise<RenderedDiff> {
  ...
  let maxLineNumber = Math.max(oldContent.split("\n").length, newContent.split("\n").length);
  ...
  for (const line of lines) {
    ...
    maxLineNumber = Math.max(maxLineNumber, oldLineNumber, newLineNumber);
  }
  ...
  for (const line of lines) {
    ...
    if (line.startsWith(" ")) {
      output.push(` ${String(oldLineNumber).padStart(lineNumberWidth, " ")} ${line.slice(1)}`);
      oldLineNumber++;
      newLineNumber++;
      continue;
    }
    ...
    if (line.startsWith("-")) {
      if (firstChangedLine === undefined) firstChangedLine = newLineNumber;
      output.push(`-${String(oldLineNumber).padStart(lineNumberWidth, " ")} ${line.slice(1)}`);
      oldLineNumber++;
      continue;
    }
    ...
    if (line.startsWith("+")) {
      if (firstChangedLine === undefined) firstChangedLine = newLineNumber;
      output.push(`+${String(newLineNumber).padStart(lineNumberWidth, " ")} ${line.slice(1)}`);
      newLineNumber++;
    }
  }
